...
set vi_driver_dir = ${vi_base}/VI_scripts/scripts_for_rt_gaea_DEV/
...
if ( ! -e $vitfiles[1] ) then
  ...
  if ( -f ${obs_vital} && -f ${ic_src_file} ) then
    ...
    #echo "${CDATE} 0" >>  ${vi_driver_dir}/vi_dev.log
  endif
...
if ( -e $vitfiles[1] ) then
  ...
  echo "${CDATE} `echo ${STORMIDlist} | wc -w`" >>  ${vi_driver_dir}/vi_dev.log
