...
# --- ENVIRONMENT & STACK SETTINGS ---
source /opt/intel/oneapi/setvars.sh > /dev/null 2>&1 # KGao 07/02/2024 fix
#ulimit -s unlimited
#export OMP_STACKSIZE=512M
#export OMP_NUM_THREADS=1         # Or set to 16 if postproc_ic is OpenMP-parallelized
#ulimit -a
...
crfactor=1.0       # Controls RDST1 in hafs_vi_split.x [orig.=1.0; can gradually incr. up to 5]
ICTILElist='7'     #TODO: Sync with vi_SHiELD.sh
# ++++++++++++++  END  OF MAIN USER SETTINGS +++++++++++++++ #

## Extract unique tile numbers from $VITASKlist
#ICTILElist=($(for item in $VITASKlist; do
#  echo "$item" | awk -F'_tile' '{print $2}'
#done | sort -n | uniq))
...
for ic_tile in "${ICTILElist[@]}"; do
  ...
  do
    ...
    export APRUNC="srun --ntasks=8 --export=ALL /usr/bin/time -v" #--cpus-per-task=16
    ...
    if [ ${do_split} -gt 0 ]; then
      ...
      echo ${gesfhr} $ibgs $vmax_vit $iflag_cold ${crfactor} | /usr/bin/time -v ./hafs_vi_split.x #[2026AUG18] MJM TESTING/TODO
      EXIT_CODE=$?; (( EXIT_CODES_SUM += ${EXIT_CODE} ))
    ...
    if [ ${do_pert} -gt 0 ]; then
      ...
      echo 6 ${BASIN} ${initopt} | /usr/bin/time -v ./hafs_vi_anl_pert.x #[2026AUG18] MJM TESTING/TODO
      EXIT_CODE=$?; (( EXIT_CODES_SUM += ${EXIT_CODE} ))
      # KGao - check if command executed successfully
      if [ "${EXIT_CODE}" -eq 0 ]; then
    ...
    if [ ${do_combine} -gt 0 ]; then
      ...
      echo ${gesfhr} ${BASIN} ${gfs_flag} ${initopt} | /usr/bin/time -v ./hafs_vi_anl_combine.x #[2026AUG18] MJM TESTING/TODO
      EXIT_CODE=$?; (( EXIT_CODES_SUM += ${EXIT_CODE} ))
      # KGao - check if command executed successfully
      if [ "${EXIT_CODE}" -eq 0 ]; then
    ...
    if [ ${do_enhance} -gt 0 ]; then
      ...
      if [ -s storm_env_new ]; then
        ...
        echo 6 ${BASIN} ${iflag_cold} | /usr/bin/time -v ./hafs_vi_anl_enhance.x #[2026AUG18] MJM TESTING/TODO
        EXIT_CODE=$?; (( EXIT_CODES_SUM += ${EXIT_CODE} ))
        # KGao - check if command executed successfully
        if [ "${EXIT_CODE}" -eq 0 ]; then
