...
export ic_file_dst=${ic_dir_dst}/gfs_data.tile7_vi.nc

# -- work dir
export work_dir_base=/lustre/f2/scratch/gfdl/Kun.Gao/tshield_vi_work_ic_v1/
...
export pert_dir_tag='_v3'
export combine_dir_tag='_v3'
...
work_dir_split=${work_dir_vi}/split_init${split_dir_tag}/
work_dir_pert=${work_dir_vi}/anl_pert_init${pert_dir_tag}/
work_dir_combine=${work_dir_vi}/anl_storm${combine_dir_tag}/
...
if [ ${do_enhance} -gt 0 ]; then
  ...
  if [ -s storm_env_new ]; then
    ...
    echo 6 ${basin} ${iflag_cold} | ./hafs_vi_anl_enhance.x > enhance_log
    # KGao - check if command executed successfully
    if [ $? -eq 0 ]; then
